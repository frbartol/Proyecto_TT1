...
Matrix& DEInteg(Matrix& func(double, Matrix&), double t, double tout, double relerr, double abserr, int n_eqn, Matrix& y){
    // maxnum = 500;
    double twou, fouru, told, epsilon, del, absdel, tend, releps, abseps, delsgn = 0.0,
    x, h, hi, temp1, term, psijm1, gamma, eta, p5eps, round, sum, absh, hold, hnew, kp1, kp2,
    km1, km2, ns, nsp1, realns, temp2, reali, temp4, nsm2, limit1, temp5, temp3, nsp2,
    limit2, temp6, tau, xold, erkm2, erkm1, erk, err, knew, rho_, erkp1, r;
    int State_, nostep, kle4, ki, kold = 0, i, ifail, k, im1, ip1;
    bool PermitTOUT, stiff, OldPermit = false, start, crash, phase1, nornd, success;
    Matrix& yout  = zeros(n_eqn,1), ypout = zeros(n_eqn,1);

    y = transpose(y);

    twou  = 2*SAT_Const::eps;
    fouru = 4*SAT_Const::eps;
